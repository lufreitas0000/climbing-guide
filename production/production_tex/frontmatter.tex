...
\CGDoubleTextSection{TheCrag \& Wikiloc}{
A Serra do Cuó está inserida nas maiores plataformas de mapeamento colaborativo do mundo. O engajamento da comunidade é vital para manter o setor seguro e atualizado.

{\color{brand_color}\CGTitleFont\fontsize{14}{16}\selectfont TheCrag}\\[2pt]
O \textbf{TheCrag} é a principal plataforma global de escalada. É altamente recomendado que você crie uma conta e registre suas ascensões (\textit{logbook}). Registrar seu progresso tem impactos reais:
\begin{itemize}
    \item Consolida a confirmação colaborativa do grau de dificuldade das vias.
    \item Mantém a comunidade alerta sobre problemas nas proteções, marimbondos, ou quebras de laca.
    \item Define a ``qualidade'' (estrelas) da via e fornece dados de quais vias têm mais repetições.
\end{itemize}
O uso do aplicativo ajuda a comunidade geral e torna a Serra do Cuó mais acessível para escaladores de fora. Escaneie o código abaixo para acessar a página oficial do pico:

\vspace{1em}
\CGQRCode[TheCrag: Serra do Cuó]{https://www.thecrag.com/en/climbing/brazil/area/14372948250}
}{
{\color{brand_color}\CGTitleFont\fontsize{14}{16}\selectfont Wikiloc}\\[2pt]
O \textbf{Wikiloc} é utilizado para mapear de forma precisa as trilhas de aproximação. Lá você encontrará as rotas de GPS, desde o acesso de carro até o estacionamento, além das caminhadas específicas para cada setor principal.

Utilize o código abaixo para acessar a lista oficial de trilhas da Serra do Cuó:

\vspace{1em}
\CGQRCode[Lista de Trilhas (Wikiloc)]{https://pt.wikiloc.com/wikiloc/user.do?id=20929616&event=favorites&listId=2256269}
}
